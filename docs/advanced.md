...
```bash
uv run line dry-run "Попроси Codex проверить LiveKit worker"
uv run line dry-run "статус"
uv run line dry-run "отмена"
```
...
uv run line agent-run \
  "Ответь одним коротким предложением: канал работает" \
