...
describe("Claude voice channel helpers", () => {
  test("formats named SSE events", () => {
    expect(formatSse("reply", { chat_id: "voice", text: "Готово" })).toBe(
      'event: reply\ndata: {"chat_id":"voice","text":"Готово"}\n\n',
    );
  });

  test("parses JSON voice payloads", async () => {
    const body = await parseVoiceBody(
      JSON.stringify({ text: "Проверь тесты", chat_id: "run" }),
      "application/json",
    );

    expect(body).toEqual({ text: "Проверь тесты", chat_id: "run" });
  });

  test("parses plain text voice payloads with default chat id", async () => {
    const body = await parseVoiceBody("статус", "text/plain");

    expect(body).toEqual({ text: "статус", chat_id: "voice" });
  });
