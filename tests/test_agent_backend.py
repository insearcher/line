...
def test_build_claude_cli_argv_uses_haiku_model_by_default() -> None:
    argv = build_claude_cli_argv(ClaudeCliConfig(), "Скажи коротко: ок")
...
def test_build_claude_cli_argv_allows_custom_voice_prompt() -> None:
    argv = build_claude_cli_argv(
        ClaudeCliConfig(system_prompt="Отвечай одним словом."),
        "статус",
    )

    assert argv[-3:] == ["--append-system-prompt", "Отвечай одним словом.", "статус"]
...
def test_claude_cli_backend_returns_trimmed_stdout(tmp_path: Path) -> None:
    ...
    async def runner(argv: list[str], cwd: Path | None, timeout_seconds: float) -> AgentProcessResult:
        ...
        return AgentProcessResult(returncode=0, stdout="  Готово.\n", stderr="")
    ...
    result = asyncio.run(backend.run("статус"))

    assert result.reply == "Готово."
    assert result.model == "haiku"
    assert calls[0][0][-1] == "статус"
...
def test_claude_cli_backend_raises_on_failed_process() -> None:
    ...
    with pytest.raises(AgentBackendError, match="model not found"):
        asyncio.run(backend.run("статус"))
...
def test_build_codex_turn_start_params_uses_text_input_and_full_access(tmp_path: Path) -> None:
    ...
    assert params["input"] == [{"type": "text", "text": "проверь тесты", "text_elements": []}]
    assert params["cwd"] == str(tmp_path)
    assert params["model"] == "gpt-5.5"
    assert params["approvalPolicy"] == "never"
...
def test_codex_app_server_backend_runs_turn_through_json_rpc_transport(tmp_path: Path) -> None:
    transport = ScriptedCodexTransport(
        [
            ...
            {"id": 3, "result": {"turn": {"id": "turn-1", "status": "running"}}},
            {
                ...
                "params": {"threadId": "thread-1", "turnId": "turn-1", "itemId": "item-1", "delta": "Готово"},
            },
            {
                "method": "item/agentMessage/delta",
    ...
    result = asyncio.run(backend.run("проверь тесты"))
...
def test_codex_app_server_backend_prefers_reply_to_voice_tool_text(tmp_path: Path) -> None:
        ...
        [
            ...
            },
            {
                ...
                "params": {
                    ...
                    "namespace": None,
                    "tool": "reply_to_voice",
                    "arguments": {"text": "Проект находится в папке MVP.", "status": "done"},
                },
            },
            {
                "method": "turn/completed",
    ...
    result = asyncio.run(backend.run("где проект?"))

    assert result.reply == "Проект находится в папке MVP."
    assert result.full_reply == "Сейчас изучаю проект."
    assert result.voice_replies == (VoiceReply(text="Проект находится в папке MVP.", status="done"),)
...
def test_codex_app_server_backend_falls_back_to_last_agent_message(tmp_path: Path) -> None:
    transport = ScriptedCodexTransport(
        [
            ...
            {
                "method": "item/agentMessage/delta",
                "params": {"threadId": "thread-1", "turnId": "turn-1", "itemId": "item-1", "delta": "Промежуточно."},
            },
            {
                "method": "item/agentMessage/delta",
                "params": {"threadId": "thread-1", "turnId": "turn-1", "itemId": "item-2", "delta": "Финал"},
            },
            {
                "method": "item/agentMessage/delta",
    ...
    result = asyncio.run(backend.run("ответь финалом"))

    assert result.reply == "Финал."
    assert result.full_reply == "Промежуточно.\n\nФинал."
...
def test_codex_app_server_backend_reuses_thread_for_next_turn(tmp_path: Path) -> None:
    transport = ScriptedCodexTransport(
        [
            ...
            {
                "method": "item/agentMessage/delta",
                "params": {"threadId": "thread-1", "turnId": "turn-1", "itemId": "item-1", "delta": "Первый"},
            },
            {
                "method": "turn/completed",
                "params": {"threadId": "thread-1", "turn": {"id": "turn-1", "status": "completed"}},
            ...
            {
                "method": "item/agentMessage/delta",
                "params": {"threadId": "thread-1", "turnId": "turn-2", "itemId": "item-2", "delta": "Второй"},
            },
            {
                "method": "turn/completed",
    ...
    first = asyncio.run(backend.run("первая команда"))
    second = asyncio.run(backend.run("вторая команда"))

    assert first.reply == "Первый"
    assert second.reply == "Второй"
...
def test_codex_app_server_backend_closes_transport_on_timeout(tmp_path: Path) -> None:
    ...
    with pytest.raises(AgentBackendError, match="timed out"):
        asyncio.run(backend.run("зависни"))
...
def test_codex_app_server_backend_resumes_existing_thread_when_thread_id_is_configured(tmp_path: Path) -> None:
    ...
    result = asyncio.run(backend.run("продолжи"))
...
def test_codex_app_server_backend_can_use_existing_websocket_endpoint(tmp_path: Path) -> None:
    async def run_case() -> tuple[str, list[dict]]:
        ...
        try:
            ...
            result = await backend.run("продолжи")
