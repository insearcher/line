...
def test_demo_server_json_endpoints(tmp_path, monkeypatch) -> None:
    ...
    event = EventLog(events_path).append("transcript.final", {"text": "привет"})
    TaskQueue(queue_path).add_task(task_text="проверить очередь", source_text="Попроси Кодекс проверить очередь")
    ...
    try:
        ...
        assert events_payload["events"][0]["payload"] == {"text": "привет"}
        ...
        assert tasks_payload["tasks"][0]["task_text"] == "проверить очередь"
        ...
        assert agent_runs_payload["agent_runs"][0]["prompt"] == "проверь тесты"
    finally:
        server.shutdown()
