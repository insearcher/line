...
def test_recognize_and_route_speech_records_usage_and_queues_task(tmp_path) -> None:
    ...
    stt = FakeSTT("Попроси Кодекс проверить очередь")
    ...
    assert result.transcript == "Попроси Кодекс проверить очередь"
...
def test_recognize_and_route_speech_speaks_route_reply(tmp_path) -> None:
    ...
    assert result.reply == "Очередь Codex пуста."
    assert speaker.spoken_texts == ["Очередь Codex пуста."]
...
def test_recognize_and_route_speech_sends_transcript_to_claude_channel(tmp_path) -> None:
    ...
    result = asyncio.run(
        lowlevel_worker.recognize_and_route_speech(
            frames=[SimpleNamespace(duration=0.5)],
            stt=FakeSTT("Попроси Claude проверить тесты"),
            queue=queue,
            events=events,
            usage=UsageLog(tmp_path / "usage.jsonl"),
    ...
    assert result.transcript == "Попроси Claude проверить тесты"
    assert result.reply == ""
    assert result.route is None
    assert client.sent == [("Попроси Claude проверить тесты", "voice")]
...
def test_recognize_and_route_speech_dispatches_to_agent_backend(tmp_path) -> None:
    ...
    backend = FakeAgentBackend(reply="Haiku ответил.")
    ...
    assert result.reply == "Принял, отправил haiku: проверь тесты."
    assert result.route is None
    assert backend.prompts == ["проверь тесты"]
    assert speaker.enqueued_texts == [
        "Принял, отправил haiku: проверь тесты.",
        "Haiku ответил.",
    ]
    assert queue.list_tasks() == []
    ...
    assert run.prompt == "проверь тесты"
    assert run.reply == "Haiku ответил."
...
def test_run_agent_backend_job_records_voice_reply_tool_payload(tmp_path) -> None:
    ...
    events = EventLog(tmp_path / "events.jsonl")
    runs = AgentRunStore(tmp_path / "agent_runs.jsonl")
    run = runs.create_queued(backend="codex", model="gpt-5.5", prompt="где проект?")
    backend = FakeAgentBackend(
        reply="Проект находится в папке MVP.",
        voice_replies=(VoiceReply(text="Проект находится в папке MVP.", status="done"),),
        full_reply="Сейчас изучаю проект.\n\nПроект находится в папке MVP.",
    )
    ...
    assert speaker.spoken_texts == ["Проект находится в папке MVP."]
    done_event = events.read_all()[-1]
    assert done_event.type == "agent_backend.done"
    assert done_event.payload["reply"] == "Проект находится в папке MVP."
    assert done_event.payload["full_reply"] == "Сейчас изучаю проект.\n\nПроект находится в папке MVP."
    assert done_event.payload["voice_replies"] == [
        {"text": "Проект находится в папке MVP.", "status": "done"}
    ]
    assert runs.list_runs()[0].reply == "Проект находится в папке MVP."


def test_recognize_and_route_speech_marker_mode_buffers_until_submit(tmp_path) -> None:
    speaker = FakeSpeechQueue()
    backend = FakeAgentBackend(reply="готово")
    ...
    assert speaker.enqueued_texts == ["готово"]
...
def test_marker_mode_ignores_prompt_text_spoken_before_start_tone(tmp_path) -> None:
    ...
    async def run_case() -> None:
        await lowlevel_worker.recognize_and_route_speech(
            frames=[SimpleNamespace(duration=0.2)],
            stt=FakeSTT("запись проверь тесты конец"),
            queue=queue,
            events=events,
            usage=UsageLog(tmp_path / "usage.jsonl"),
            tts_publisher=speaker,
...
def test_marker_probe_starts_capture_before_regular_vad_end(tmp_path) -> None:
    ...
    async def run_case() -> bool:
        return await lowlevel_worker.probe_for_capture_start(
            ...
            stt=FakeSTT("фоновые слова запись"),
            capture_session=capture_session,
            events=events,
            usage=UsageLog(tmp_path / "usage.jsonl"),
            tts_publisher=speaker,
...
def test_marker_probe_ignores_non_marker_transcript(tmp_path) -> None:
    ...
    async def run_case() -> bool:
        return await lowlevel_worker.probe_for_capture_start(
            frames=[SimpleNamespace(duration=1.0)],
            stt=FakeSTT("просто фоновая речь"),
            capture_session=capture_session,
            events=events,
            usage=UsageLog(tmp_path / "usage.jsonl"),
            tts_publisher=speaker,
...
def test_marker_probe_ignores_active_capture_text_without_cancel(tmp_path) -> None:
    speaker = FakeSpeechQueue()
    capture_session = MarkerCaptureSession(CaptureConfig(mode=CaptureMode.MARKERS))
    capture_session.handle_text("запись", now=9.0)
...
def test_marker_probe_cancels_active_capture_before_regular_vad_end(tmp_path) -> None:
    ...
    capture_session.handle_text("запись", now=9.0)
    capture_session.mark_ready(now=9.5)
    capture_session.handle_text("проверь тесты", now=10.0)
    events = EventLog(tmp_path / "events.jsonl")
...
def test_marker_probe_cancels_active_capture_with_clipped_otmena(tmp_path) -> None:
    speaker = FakeSpeechQueue()
    capture_session = MarkerCaptureSession(CaptureConfig(mode=CaptureMode.MARKERS))
    capture_session.handle_text("запись", now=9.0)
    capture_session.mark_ready(now=9.5)
...
def test_marker_probe_ignores_active_submit_marker(tmp_path) -> None:
    ...
    capture_session.handle_text("запись", now=9.0)
    capture_session.mark_ready(now=9.5)
    capture_session.handle_text("проверь", now=10.0)
    agent_tasks: set[asyncio.Task] = set()
    events = EventLog(tmp_path / "events.jsonl")
    runs = AgentRunStore(tmp_path / "agent_runs.jsonl")
...
def test_capture_timeout_cancels_buffer(tmp_path) -> None:
    ...
    capture_session.handle_text("запись", now=10.0)
    capture_session.mark_ready(now=10.5)
    capture_session.handle_text("проверь тесты", now=11.0)
...
def test_control_reset_clears_active_capture_session(tmp_path) -> None:
    events = EventLog(tmp_path / "events.jsonl")
    capture_session = MarkerCaptureSession(CaptureConfig(mode=CaptureMode.MARKERS))
    capture_session.handle_text("запись", now=0.0)
    capture_session.mark_ready(now=0.5)
    capture_session.handle_text("проверь тесты", now=1.0)
...
def test_consume_claude_replies_speaks_sse_replies(tmp_path) -> None:
    ...
    asyncio.run(
        lowlevel_worker.consume_claude_replies(
            reply_stream=FakeClaudeReplyStream([ClaudeReply(chat_id="voice", text="Готово", status="done")]),
            tts_publisher=speaker,
            events=events,
            max_replies=1,
        )
    ...
    assert speaker.spoken_texts == ["Готово"]
...
def test_build_deepgram_stt_passes_owned_http_session() -> None:
    ...
    voice_config = SimpleNamespace(deepgram_api_key=stt_api_key, deepgram_language="ru")
    session = object()
...
def test_extract_final_transcript_reads_first_alternative() -> None:
    assert lowlevel_worker.extract_final_transcript(FakeSpeechEvent("текст")) == "текст"
