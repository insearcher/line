...
def test_send_voice_message_posts_json_with_sender_header() -> None:
    ...
    try:
        ...
        response = client.send_voice_message("Проверь тесты", chat_id="run")
    finally:
        server.shutdown()
    ...
    assert received["body"] == {"text": "Проверь тесты", "chat_id": "run"}
...
def test_parse_sse_payload_returns_reply_events_only() -> None:
    payload = (
        ...
        "event: reply\n"
        'data: {"chat_id":"voice","text":"Принял","status":"ack","created_at":"2026-05-02T10:00:00Z"}\n\n'
    )
