...
def test_tts_publisher_publishes_track_and_captures_synthesized_frames(tmp_path) -> None:
    ...
    result = asyncio.run(_start_and_say(publisher, "Принял задачу."))
    ...
    assert usage.monthly_summary(result.created_at[:7])["tts.chars"]["chars"] == len("Принял задачу.")
