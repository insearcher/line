...
def test_dry_run_dispatch_writes_queue(tmp_path: Path) -> None:
    result = run_dry_run(
        text="Попроси Codex проверить LiveKit worker",
    ...
    assert payload["queued_task"]["task_text"] == "проверить livekit worker"
    assert payload["spoken_reply"].startswith("Принял")


def test_dry_run_cancel_does_not_write_queue(tmp_path: Path) -> None:
    queue_path = tmp_path / "tasks.jsonl"

    result = run_dry_run(text="отмена", queue_path=queue_path)
...
def test_parser_accepts_lowlevel_worker_command() -> None:
    ...
    assert args.capture_wake_word == "кодекс"
    assert args.capture_start == "начало"
    assert args.capture_submit == "конец команды"
    assert args.capture_cancel == "отмена"
...
def test_parser_accepts_claude_channel_send_command() -> None:
    ...
    assert args.text == "Проверь тесты"
...
def test_run_agent_once_closes_backend_after_reply() -> None:
    ...
    result = asyncio.run(run_agent_once(backend, "статус"))
