...
class CaptureConfig:
    ...
    wake_word: str = ""
    start_phrase: str = "запись"
    submit_phrase: str = "конец"
    cancel_phrase: str = "отмена"
...
class MarkerCaptureSession:
    ...
    def handle_cancel_text(self, text: str) -> CaptureAction:
        ...
        if self._find_control(normalized, kinds={CaptureActionKind.CANCELLED}) is None:
            return CaptureAction(kind=CaptureActionKind.IGNORE, text=text)
        return self._cancel(text=text, reason="explicit_cancel", reply="Команда отменена.")
    ...
    def check_timeout(self, *, now: float) -> CaptureAction:
        ...
        return self._cancel(text="", reason="silence_timeout", reply="Команда отменена из-за паузы.")
