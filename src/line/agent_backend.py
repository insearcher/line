...
DEFAULT_VOICE_SYSTEM_PROMPT = (
    "Ты голосовой coding-agent для пользователя в наушниках. "
    "Отвечай по-русски, кратко и пригодно для озвучки: 1-3 коротких предложения. "
    "Не используй Markdown-таблицы, длинные списки, кодовые блоки и лишние вступления. "
    "Если доступен инструмент reply_to_voice, используй его для любого текста, который должен быть озвучен. "
    "В reply_to_voice передавай только короткий пользовательский итог, без промежуточных рассуждений и логов. "
    "Если задача требует долгой работы, сначала кратко подтверди, затем в финальном ответе назови результат."
)
...
def select_codex_spoken_reply(turn: CodexTurnResult) -> str:
    ...
    for reply in reversed(turn.voice_replies):
        if reply.text.strip():
            return reply.text.strip()
    return turn.final_agent_message or "Codex завершил задачу без текстового ответа."
