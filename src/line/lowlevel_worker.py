...
async def run_agent_backend_job(
    *,
    ...
    try:
        ...
        message = f"{label} вернул ошибку: {format_error(error)}"
        events.append(
        ...
        message = f"{label} упал: {format_error(error)}"
        events.append(
            "agent_backend.failed",
            {"backend": label, "run_id": run.id if run else None, "text": prompt, "error": format_error(error)},
        )
...
def build_agent_ack(*, label: str, prompt: str) -> str:
    return f"Принял, отправил {label}: {shorten_for_voice(prompt, limit=120)}."
